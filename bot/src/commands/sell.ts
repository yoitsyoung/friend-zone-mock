...
import { storeSellShare } from "../contracts/FriendZoneShare";

export async function handleSellCommand(msg: TelegramBot.Message) {
  ...
  if (!amount) {
    await bot.sendMessage(
      groupChatId,
      "Please specify amount of shares to sell"
    );
    return;
  }
          ...
          payload: beginCell()
            .store(
              storeSellShare({
                ...
                seller: address(wallet.account.address),
              })
            )
            .endCell()
            .toBoc()
